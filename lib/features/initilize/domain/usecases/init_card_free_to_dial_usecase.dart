...
class InitCardFreeToDialUsecase implements Usecase<String,InitCardFreeToDialUsecaseParams>{
  final InitCardRepository initCardRepository;
  InitCardFreeToDialUsecase({required this.initCardRepository});
  @override
  Future<Either<Failure, String>> call(InitCardFreeToDialUsecaseParams params) async {
    return await initCardRepository.initCardFreeToDial(cardDetails:InitCardFreeToDialEntity(signal: params.signal, mode: params.mode, amount: params.amount) );
  }
}
...
class InitCardFreeToDialUsecaseParams{
  ...
  final String mode;
  final String amount;
  InitCardFreeToDialUsecaseParams({required this.signal , required this.amount , required this.mode});
}