...
class InitCardFreeToDialCubit extends Cubit<InitCardFreeToDialState> {
  ...
  }) : _initCardFreeToDialUsecase = initCardFreeToDialUsecase,
       super(InitCardFreeToDialInitial());

  Future<void> initCardFreeToDial({required String amount}) async {
    emit(InitCardFreeToDialLoadingState());
    final res = await _initCardFreeToDialUsecase(
      InitCardFreeToDialUsecaseParams(signal: "2", amount: amount, mode: "1"),
    );
    res.fold(
      (failure) {
